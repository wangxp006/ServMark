...
#define NUM_VERIFIES 20000
#define MSG_SIZE 32

/*
 * ECDSA P-256 SHA-256 verify. TLS server cert chain verification.
 * Verify is ~2-5x faster than sign (vs RSA where verify is ~100x faster).
 */
...
static int crypto_ecdsa_p256_verify_init(void **state) {
    ecdsa_vfy_state_t *s = calloc(1, sizeof(*s));
    if (!s) return -1;
...
static int crypto_ecdsa_p256_verify_warmup(void *state) {
    ecdsa_vfy_state_t *s = (ecdsa_vfy_state_t *)state;
    EVP_MD_CTX *ctx = EVP_MD_CTX_new();
    if (!ctx) return -1;
...
static int crypto_ecdsa_p256_verify_measure(void *state, measurement_t *result) {
    ecdsa_vfy_state_t *s = (ecdsa_vfy_state_t *)state;
...
static int crypto_ecdsa_p256_verify_cleanup(void *state) {
    ecdsa_vfy_state_t *s = (ecdsa_vfy_state_t *)state;
    EVP_PKEY_free(s->pkey); free(s->sig); free(s); return 0;
}

benchmark_t bench_crypto_ecdsa_p256_verify = {
    .name="crypto-ecdsa-p256-verify", .category="C3",
    .description="ECDSA P-256 SHA-256 verify (TLS cert chain, nistz256 optimized)",
    ...
    .init=crypto_ecdsa_p256_verify_init, .warmup=crypto_ecdsa_p256_verify_warmup,
    .measure=crypto_ecdsa_p256_verify_measure, .cleanup=crypto_ecdsa_p256_verify_cleanup,
    .num_threads=1,
};
SSB_BENCHMARK_REGISTER(bench_crypto_ecdsa_p256_verify);
