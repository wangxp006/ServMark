...
/*
 * SM2 signing (Chinese national standard GM/T 0003).
 * Mandated for Chinese financial systems, e-government, critical infrastructure.
 * SM2 curve + SM3 hash. OpenSSL >= 1.1.1 native support.
 */
...
static int crypto_sm2_sign_init(void **state) {
    sm2_state_t *s = calloc(1, sizeof(*s));
    if (!s) return -1;
...
static int crypto_sm2_sign_warmup(void *state) {
    sm2_state_t *s = (sm2_state_t *)state;
    EVP_MD_CTX *ctx = EVP_MD_CTX_new();
    if (!ctx) return -1;
    size_t slen = s->sig_len;
    int ret = EVP_DigestSignInit(ctx, NULL, EVP_sm3(), NULL, s->pkey);
    if (ret == 1) EVP_DigestSign(ctx, s->sig, &slen, s->message, MSG_SIZE);
    EVP_MD_CTX_free(ctx);
    return (ret == 1) ? 0 : -1;
}

static int crypto_sm2_sign_measure(void *state, measurement_t *result) {
    sm2_state_t *s = (sm2_state_t *)state;
    ...
    for (int i = 0; i < NUM_SIGNS; i++) {
        size_t slen = s->sig_len;
        ret = EVP_DigestSignInit(ctx, NULL, EVP_sm3(), NULL, s->pkey);
        if (ret != 1) goto err;
...
static int crypto_sm2_sign_cleanup(void *state) {
    sm2_state_t *s = (sm2_state_t *)state;
    EVP_PKEY_free(s->pkey); free(s->sig); free(s); return 0;
}

benchmark_t bench_crypto_sm2_sign = {
    .name="crypto-sm2-sign", .category="C3",
    .description="SM2 SM3 signature (GM/T 0003, Chinese financial/e-government standard)",
    .tier=2, .primary_metric_name="signatures/sec", .higher_is_better=true,
    ...
    .init=crypto_sm2_sign_init, .warmup=crypto_sm2_sign_warmup,
    .measure=crypto_sm2_sign_measure, .cleanup=crypto_sm2_sign_cleanup,
    .num_threads=1,
};
SSB_BENCHMARK_REGISTER(bench_crypto_sm2_sign);
