...
/*
 * ECDSA P-256 (secp256r1) SHA-256 signing. FIPS-mandated curve:
 * AWS KMS / GCP KMS / Azure Key Vault default, enterprise TLS, JWT ES256.
 */
...
static int crypto_ecdsa_p256_sign_init(void **state) {
    ecdsa_state_t *s = calloc(1, sizeof(*s));
    if (!s) return -1;
...
static int crypto_ecdsa_p256_sign_warmup(void *state) {
    ecdsa_state_t *s = (ecdsa_state_t *)state;
    EVP_MD_CTX *ctx = EVP_MD_CTX_new();
    if (!ctx) return -1;
    size_t slen = s->sig_len;
    int ret = EVP_DigestSignInit(ctx, NULL, EVP_sha256(), NULL, s->pkey);
    if (ret == 1) EVP_DigestSign(ctx, s->sig, &slen, s->message, MSG_SIZE);
    EVP_MD_CTX_free(ctx);
    return (ret == 1) ? 0 : -1;
}

static int crypto_ecdsa_p256_sign_measure(void *state, measurement_t *result) {
    ecdsa_state_t *s = (ecdsa_state_t *)state;
...
static int crypto_ecdsa_p256_sign_cleanup(void *state) {
    ecdsa_state_t *s = (ecdsa_state_t *)state;
    EVP_PKEY_free(s->pkey); free(s->sig); free(s); return 0;
}

benchmark_t bench_crypto_ecdsa_p256_sign = {
    .name="crypto-ecdsa-p256-sign", .category="C3",
    .description="ECDSA P-256 SHA-256 signing (FIPS-compliant, AWS KMS default)",
    ...
    .init=crypto_ecdsa_p256_sign_init, .warmup=crypto_ecdsa_p256_sign_warmup,
    .measure=crypto_ecdsa_p256_sign_measure, .cleanup=crypto_ecdsa_p256_sign_cleanup,
    .num_threads=1,
};
SSB_BENCHMARK_REGISTER(bench_crypto_ecdsa_p256_sign);
