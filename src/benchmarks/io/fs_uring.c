...
/*
 * io_uring-style random read benchmark (synchronous fallback).
 * Uses pread() as portable approximation. True io_uring kernel support
 * would require ring buffer setup — see CLAUDE.md.
 */
...
static int fs_uring_init(void **state) {
    ...
    int mb = 64*1024*1024 / SSB_BLOCK_SIZE;
    for (int i = 0; i < NUM_IOS; i++)
        s->offsets[i] = (rand() % mb) * SSB_BLOCK_SIZE;
    *state = s; return 0;
}
...
benchmark_t bench_fs_uring = {
    ...
    .init = fs_uring_init, .warmup = fs_uring_warmup,
    .measure = fs_uring_measure, .cleanup = fs_uring_cleanup,
    .num_threads = 1,
};
