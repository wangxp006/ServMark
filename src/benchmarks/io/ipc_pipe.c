...
static int ipc_pipe_warmup(void *state) {
    ...
    /* Warmup using actual state pipe_fd so the right pipe is primed */
    pid_t pid = fork();
    if (pid == 0) {
        close(s->pipe_fd[1]);
        char buf[MSG_SIZE];
        for (int i = 0; i < 1000; i++) {
            if (read(s->pipe_fd[0], buf, MSG_SIZE) <= 0) break;
        }
        close(s->pipe_fd[0]); _exit(0);
    }
    if (pid > 0) {
        close(s->pipe_fd[0]);
        for (int i = 0; i < 1000; i++)
    ...
    if (pipe(s->pipe_fd) != 0) return -1; /* fresh pipe for measure() */
    return 0;
}
...
static int ipc_pipe_measure(void *state, measurement_t *result) {
    ...
    if (pid == 0) {
        close(s->pipe_fd[1]);
        char buf[MSG_SIZE]; int64_t bytes = 0;
    ...
    result->primary_metric = (double)total_bytes / el;
...
static int ipc_pipe_cleanup(void *state) {
    ...
    free(s->buffer); free(s); return 0;
}

benchmark_t bench_ipc_pipe = {
    .name="ipc-pipe", .category="C11",
    ...
    .init=ipc_pipe_init, .warmup=ipc_pipe_warmup,
    .measure=ipc_pipe_measure, .cleanup=ipc_pipe_cleanup,
    .num_threads=1,
};
