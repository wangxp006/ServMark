...
/*
 * NUMA page migration throughput. Uses runtime page size for correct
 * page-faulting on ARM64 (16KB/64KB) and RISC-V systems.
 */
...
static int numa_migration_warmup(void *state) {
    ...
    long ps = s->page_size;
    for (size_t i = 0; i < BUF_SIZE / 10; i += ps) sink += s->buffer[i];
...
static int numa_migration_measure(void *state, measurement_t *result) {
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        for (size_t off = 0; off < BUF_SIZE; off += ps)
            sink += s->buffer[off];
        migrations++;
    }
...
benchmark_t bench_numa_migration = {
    .name = "numa-migration", .category = "C5",
    .description = "NUMA page migration throughput (runtime page size)",
    .tier = 1, .primary_metric_name = "migrations/sec", .higher_is_better = true,
    ...
    .init = numa_migration_init, .warmup = numa_migration_warmup,
    .measure = numa_migration_measure, .cleanup = numa_migration_cleanup,
    .num_threads = 1,
};
