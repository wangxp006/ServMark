...
typedef struct {
    char *is_prime;
} vm_cpu_overhead_state_t;

static int sieve_of_eratosthenes(char *is_prime, int limit) {
...
static int vm_cpu_overhead_warmup(void *state) {
    ...
    volatile int r = sieve_of_eratosthenes(s->is_prime, SIEVE_LIMIT / 4);
...
static int vm_cpu_overhead_measure(void *state, measurement_t *result) {
    ...
    for (int pass = 0; pass < SIEVE_PASSES; pass++)
        total_primes += sieve_of_eratosthenes(s->is_prime, SIEVE_LIMIT);

    clock_gettime(CLOCK_MONOTONIC, &t1);
    __asm__ __volatile__("" : "+r"(total_primes));

    double elapsed = (t1.tv_sec - t0.tv_sec) + (t1.tv_nsec - t0.tv_nsec) / 1e9;
    memset(result, 0, sizeof(*result));
    result->primary_metric = (double)SIEVE_PASSES / elapsed;
...
static int vm_cpu_overhead_cleanup(void *state) {
    vm_cpu_overhead_state_t *s = (vm_cpu_overhead_state_t *)state;
    free(s->is_prime); free(s);
    return 0;
}

benchmark_t bench_vm_cpu_overhead = {
    .name = "vm-cpu-overhead",
    .category = "C14",
    .description = "CPU virtualization overhead (Sieve of Eratosthenes fixed work)",
    .tier = 1,
    .primary_metric_name = "passes/sec",
    ...
    .init = vm_cpu_overhead_init,
    .warmup = vm_cpu_overhead_warmup,
    .measure = vm_cpu_overhead_measure,
    .cleanup = vm_cpu_overhead_cleanup,
    .num_threads = 1,
};
